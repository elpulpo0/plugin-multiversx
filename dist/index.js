...
var NativeAuthProvider = class {
   ...
   */
  constructor(config2) {
    this.client = new NativeAuthClient(config2);
  }
  /**
...
function isUserAuthorized(userId, runtime) {
  ...
  elizaLogger2.log("UserID from message:", userId);
  elizaLogger2.log("Authorized UserID:", authorizedUserId);
...
var transfer_default = {
  ...
  validate: async (runtime, message) => {
    elizaLogger3.log("Validating config for user:", message.userId);
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    elizaLogger3.log("Handler initialized. Checking user authorization...");
...
var createToken_default = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger4.log("Starting CREATE_TOKEN handler...");
    ...
    if (!isCreateTokenContent) {
      elizaLogger4.error("Invalid content for CREATE_TOKEN action.");
      if (callback) {
        callback({
          text: "Unable to process transfer request. Invalid content provided.",
          content: { error: "Invalid transfer content" }
    ...
    try {
      ...
      elizaLogger4.error("Error during creating token:", error);
      if (callback) {
        callback({
...
var swap_default = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger5.log("Starting SWAP handler...");
    ...
    if (!isSwapContent) {
      elizaLogger5.error("Invalid content for SWAP action.");
      callback?.({
    ...
    try {
      ...
      elizaLogger5.error("Error during token swap:", error);
      callback?.({
...
var multiversxPlugin = {
  ...
  actions: [transfer_default, createToken_default, swap_default],
  evaluators: [],
  providers: []
};
